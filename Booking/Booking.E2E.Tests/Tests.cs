...
namespace Booking.Module.E2E.Tests;

public class BookingTests : IDisposable {
    const string BlazorAppName = "BookingBlazor";
    const string WinAppName = "BookingWin";
    const string AppDBName = "Booking";
    EasyTestFixtureContext FixtureContext { get; } = new EasyTestFixtureContext();

	public BookingTests() {
        FixtureContext.RegisterApplications(
            new BlazorApplicationOptions(BlazorAppName, string.Format(@"{0}\..\..\..\..\Booking.Blazor.Server", Environment.CurrentDirectory)),
            new WinApplicationOptions(WinAppName, string.Format(@"{0}\..\..\..\..\Booking.Win\bin\EasyTest\net8.0-windows\Booking.Win.exe", Environment.CurrentDirectory))
        );
        FixtureContext.RegisterDatabases(new DatabaseOptions(AppDBName, "BookingEasyTest", server: @"(localdb)\mssqllocaldb"));	           
	}
    public void Dispose() {
